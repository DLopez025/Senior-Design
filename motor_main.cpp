...
// enum Control {Control_INIT, Control_wait, Manual_Start, Manual_Restart, Auto_Start} Control_state;
// enum Track {Track_INIT, Track_wait} Track_state;
...
// const byte address[4][6] = {"00001", "00002","00003", "00004"}; //From LCD, From Joystick, TO LCD, from Camera
...
void setup() {
  ...
  // radio.openReadingPipe(2, Campipe); //from Camera
  // radio.openReadingPipe(3, address[3]); //from camera
  radio.openWritingPipe(Motorpipe); //To Motor
  ...
  myservo.write(0);
  servo_2.write(180);
  // tasks[0].state = Control_INIT;
  // tasks[0].TickFct = &Control;
}
...
void loop() { 
  ...
  if(Manual == 1){
    ...
    // if(lastx != clawposx || lasty != clawposy){
    //   // Serial.print("Clawposx: ");
    //   // Serial.println(clawposx);
    //   // Serial.print("Clawposy: ");
    //   // Serial.println(clawposy);
    //   lastx = clawposx;
    //   lasty = clawposy;
    // }
  if(Return == 1){
